...
def start_server(fpath):
    ...
    # Using temporary_directory as a temporary directory
    TEMP_DIR_NAME = "temporary_directory"

    # Remove quotation marks ("") and replace with nothing from directory
    fpath = fpath.replace("\"", "")
    ...
    # Makes a directory named temporary_directory and stores the file there
    if os.path.isdir(TEMP_DIR_NAME):
    # if the temporary directory exists already, delete it
    ...
    try:
        ...
        # If given path points to a file copy it to temporary directory
            ...
            # Copy the file to temporary_directory
            copy2(fpath, fname)
    ...
    # Change our directory to .temporary_directory
...
def print_qr_code(address, qrfname):
    ...
    if  os.name == 'nt':        # Check if the machine is running Windows
        ...
        os.system(qrfname)
        print("If you want the QR code picture, you can temporarily find it under the temporary_directory directory.\n\n")
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        print("\nTransfer files over the same network from your computer to your mobile device by scanning a QR code.")
        fpath = input("Enter the file/folder's directory or drag and drop it here: ")
